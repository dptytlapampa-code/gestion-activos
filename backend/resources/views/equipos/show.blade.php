...
@php
    ...
    $maintenanceDocumentModalInicial = str_starts_with($documentContext, 'mantenimiento:')
        ? (int) substr($documentContext, strlen('mantenimiento:'))
        : null;
    $canManageMaintenanceDocuments = auth()->user()->hasRole(
        \App\Models\User::ROLE_SUPERADMIN,
        \App\Models\User::ROLE_ADMIN,
        \App\Models\User::ROLE_TECNICO,
    );
...
<div
    ...
    x-data="{
        ...
        openMaintenanceDocumentModal(id) {
            ...
            this.$nextTick(() => {
                const field = document.getElementById(`mantenimiento-type-${id}`);
                const modal = document.getElementById(`mantenimiento-document-modal-${id}`);
                (field ?? modal)?.focus();
            });
        },
    ...
    <div class="card">
        ...
        <div class="pt-6">
            ...
            @foreach($mantenimientos as $mantenimiento)
                @include('equipos.partials.mantenimiento-document-modal', [
                    'mantenimiento' => $mantenimiento,
                    'documentContext' => $documentContext,
                    'canManageMaintenanceDocuments' => $canManageMaintenanceDocuments,
                ])
            @endforeach
        </div>
    </div>
