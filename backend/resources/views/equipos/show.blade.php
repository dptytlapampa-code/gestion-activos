    ...
    <div class="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
        <div class="border-b border-slate-200">
            <nav class="-mb-px flex flex-wrap gap-2 md:gap-6">
                <button type="button" @click="activeTab = 'informacion'" :class="activeTab === 'informacion' ? 'tab-active-theme' : 'border-transparent text-slate-500 hover:border-slate-300 hover:text-slate-700'" class="border-b-2 px-1 pb-3 text-sm font-semibold transition">InformaciÃ³n</button>
                <button type="button" @click="activeTab = 'mantenimiento'" :class="activeTab === 'mantenimiento' ? 'tab-active-theme' : 'border-transparent text-slate-500 hover:border-slate-300 hover:text-slate-700'" class="border-b-2 px-1 pb-3 text-sm font-semibold transition">Mantenimiento</button>
                <button type="button" @click="activeTab = 'movimientos'" :class="activeTab === 'movimientos' ? 'tab-active-theme' : 'border-transparent text-slate-500 hover:border-slate-300 hover:text-slate-700'" class="border-b-2 px-1 pb-3 text-sm font-semibold transition">Movimientos</button>
                <button type="button" @click="activeTab = 'documentos'" :class="activeTab === 'documentos' ? 'tab-active-theme' : 'border-transparent text-slate-500 hover:border-slate-300 hover:text-slate-700'" class="border-b-2 px-1 pb-3 text-sm font-semibold transition">Documentos</button>
            </nav>
        </div>
        ...
        <div class="pt-6">
            <section x-show="activeTab === 'informacion'" x-cloak class="space-y-4">
                <dl class="grid gap-4 md:grid-cols-2">
                    ...
                    <div class="rounded-lg border border-slate-200 bg-slate-50/60 p-4">
                        <dt class="text-xs font-semibold uppercase tracking-wide text-slate-500">UbicaciÃ³n</dt>
            ...
            <section x-show="activeTab === 'mantenimiento'" x-cloak class="space-y-5">
                @can('create', \App\Models\Mantenimiento::class)
                    <div>
                        <button type="button" @click="showMantenimientoForm = !showMantenimientoForm" class="inline-flex items-center gap-2 rounded-lg bg-primary-theme px-4 py-2 text-sm font-semibold text-white transition hover-bg-primary-theme">
                    ...
                    <div x-show="showMantenimientoForm" class="rounded-lg border border-slate-200 bg-slate-50/60 p-4">
                        <form method="POST" action="{{ route('equipos.mantenimientos.store', $equipo) }}" class="grid gap-4 md:grid-cols-2">
                            ...
                            <div class="md:col-span-2">
                                <label class="text-sm font-medium text-slate-700">TÃ­tulo</label>
                            ...
                            <div class="md:col-span-2">
                                <button class="rounded-lg bg-primary-theme px-4 py-2 text-sm font-semibold text-white hover-bg-primary-theme">Registrar mantenimiento</button>
                            </div>
                        </form>
                    </div>
                @endcan
                    ...
                    <table class="min-w-full text-sm">
                        <thead class="bg-slate-50 text-left text-xs uppercase text-slate-600">
                            <tr>
                                ...
                                <th class="px-4 py-3">DuraciÃ³n</th>
                        ...
                        @forelse($mantenimientos as $mantenimiento)
                            <tr class="transition hover:bg-slate-50/80">
                                ...
                                <td class="px-4 py-4 text-slate-700">{{ $mantenimiento->dias_en_servicio !== null ? $mantenimiento->dias_en_servicio.' dÃ­as' : '-' }}</td>
                                <td class="px-4 py-4">
                                    <div class="flex items-center gap-2">
                                        ...
                                        @can('delete', $mantenimiento)
                                            <form method="POST" action="{{ route('mantenimientos.destroy', $mantenimiento) }}" class="inline" onsubmit="return confirm('Â¿Eliminar mantenimiento?')">
                                                @csrf
                                                @method('DELETE')
            ...
            <section x-show="activeTab === 'movimientos'" x-cloak class="space-y-5">
                @can('update', $equipo)
                    <div class="flex justify-end">
                        <a href="{{ route('movimientos.create', ['equipo_id' => $equipo->id]) }}" class="rounded-lg bg-primary-theme px-4 py-2 text-sm font-semibold text-white transition hover-bg-primary-theme">
                    ...
                    <table class="min-w-full divide-y divide-slate-200 text-sm">
                        <thead class="bg-slate-50">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left font-semibold text-slate-700">ObservaciÃ³n</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100 bg-white">
                                    ...
                                    <td class="px-4 py-4 text-slate-700">
                                        ...
                                        <div class="mt-2 space-y-1">
                                            @foreach($movimiento->documents as $documento_mov)
                                                <div class="text-xs">
                                                    <a class="inline-flex items-center gap-1 text-primary-theme" href="{{ route('documents.download', $documento_mov) }}" title="{{ $documento_mov->original_name }}">
                                        ...
                                        @if(auth()->user()->hasRole(\App\Models\User::ROLE_SUPERADMIN, \App\Models\User::ROLE_ADMIN, \App\Models\User::ROLE_TECNICO))
                                            <form method="POST" action="{{ route('movimientos.documents.store', $movimiento) }}" enctype="multipart/form-data" class="mt-3 flex flex-wrap items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 p-2">
                                                ...
                                                <button class="inline-flex items-center rounded p-1 text-primary-theme hover-bg-primary-soft-theme" title="Adjuntar documento">
            ...
            <section x-show="activeTab === 'documentos'" x-cloak class="space-y-5">
                @if(auth()->user()->hasRole(\App\Models\User::ROLE_SUPERADMIN, \App\Models\User::ROLE_ADMIN, \App\Models\User::ROLE_TECNICO))
                    <div>
                        <button type="button" @click="showDocumentoForm = !showDocumentoForm" class="inline-flex items-center gap-2 rounded-lg bg-primary-theme px-4 py-2 text-sm font-semibold text-white transition hover-bg-primary-theme">
                            <span class="text-lg leading-none">+</span>
                            Adjuntar documento
                        </button>
                    </div>
                    ...
                    <div x-show="showDocumentoForm" class="rounded-lg border border-slate-200 bg-slate-50/60 p-4">
                        <form method="POST" action="{{ route('equipos.documents.store', $equipo) }}" enctype="multipart/form-data" class="grid gap-3 md:grid-cols-4">
                            ...
                            <button class="rounded bg-primary-theme px-3 py-2 text-white">Subir</button>
                        </form>
                    </div>
                @endif
                ...
                <div class="space-y-3">
                    @forelse($equipo->documents as $document)
                        <article class="flex flex-col gap-3 rounded-lg border border-slate-200 bg-white p-4 shadow-sm md:flex-row md:items-center md:justify-between">
                            ...
                            <div class="flex items-center gap-2">
                                <a class="rounded-lg border border-primary-soft-theme px-3 py-1.5 text-sm font-medium text-primary-theme transition hover-bg-primary-soft-theme" href="{{ route('documents.download', $document) }}">Descargar</a>
                                @can('delete', $document)
